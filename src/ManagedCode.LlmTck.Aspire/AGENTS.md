...
Extension methods should be declarative and should configure the package-owned `LlmTckResource` through Aspire primitives. Consumer examples must use `builder.AddLlmTck()` without service project paths, generated `Projects.*` metadata types, Docker, or a container runtime. Keep container hosting as an explicit opt-in API such as `builder.AddLlmTckContainer()`, not the default local AppHost path. Every behavior change needs an Aspire integration test that starts the AppHost.
