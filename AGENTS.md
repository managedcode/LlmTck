...
- The browser admin panel must be served from `/`, and runtime control APIs must use `/admin-api/...` paths; do not put `llm-tck` in runtime URL paths or add `__llm-tck`-style hidden routes for reset, configure, models, assertions, or operator UI.
- The browser admin panel must be implemented as Blazor server-side rendered components in `ManagedCode.LlmTck.Hosting`; do not serve it from a static HTML string endpoint.
...
- Remove legacy browser admin pages and old admin route aliases instead of keeping compatibility shells; stale `/admin/...`, `/admin/llm-tck/...`, or `__llm-tck` admin surfaces must stay unmapped.
