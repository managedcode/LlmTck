...
Aspire integration adds the TCK through `builder.AddLlmTck()` without requiring consumer AppHosts to reference a service project or generated `Projects.*` metadata type. The package-owned resource uses the versioned `ghcr.io/managedcode/llm-tck` container image. Compatibility is selected with fluent methods such as `.WithOpenAICompatibility()`, `.WithAzureOpenAICompatibility()`, or `.WithAnthropicCompatibility()`. Consumer resources reference the `LlmTckResource`, wait for it, and read its `http` endpoint through `GetHttpEndpoint()` or Aspire's `GetEndpoint("http")`.
