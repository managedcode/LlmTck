...
| Aspire AppHost is built in test code, adds the package-owned TCK with `builder.AddLlmTck()`, configures endpoint/API key/provider compatibility, starts the container-backed service, and serves implemented modalities plus official Azure SDK clients | `AspireIntegrationTests.AddLlmTck_BuildsAppHostInTestAndSupportsConfiguredClientsAsync` |
